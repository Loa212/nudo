...
pub async fn serve(config: Config, addr: std::net::SocketAddr) -> anyhow::Result<()> {
    ...
    let store = nudo_server::store::Store::open(&config.database).await?;
    let bus = nudo_server::events::Bus::new(config.log_buffer_lines);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for target in targets {
                ...
                // else.
                let status = match nudo_server::ssh::SshSession::connect(&ssh_target).await {
                    Ok(session) => {
...
/// without bound.
fn spawn_terminal_sweeper(store: nudo_server::store::Store) {
    tokio::spawn(async move {
...
async fn reconcile_interrupted_deployments(store: &nudo_server::store::Store) {
